...
class SentimentAnalysisOutput(BaseModel):
    ...
    explanation: Optional[str] = Field(
        None, description="Optional rationale behind the sentiment classification"
    )

    # Define explicit model config to control required fields
    model_config = {
        "json_schema_extra": {"required": ["sentiment", "confidence_score"]}
    }
...
class TopicCategorizationOutput(BaseModel):
    ...
    # Define explicit model config to control required fields
    model_config = {"json_schema_extra": {"required": ["primary_topic"]}}
...
class KeywordContextualizationOutput(BaseModel):
    ...
    # Define explicit model config to control required fields
    model_config = {
        "json_schema_extra": {"required": ["keyword", "context", "relevance_score"]}
    }
...
class KeywordContextualizationResult(BaseModel):
    ...
    # Define explicit model config to control required fields
    model_config = {"json_schema_extra": {"required": ["extracted_keywords"]}}
...
class SummaryGenerationOutput(BaseModel):
    ...
    # Define explicit model config to control required fields
    model_config = {"json_schema_extra": {"required": ["summary_text"]}}


# --- Tool Executor Output ---
class ToolExecutorOutput(BaseModel):
    use_tools: List[str] = Field(..., description="List of tools to use for processing")
    ...
    # Define explicit model config to control required fields
    model_config = {"json_schema_extra": {"required": ["use_tools"]}}
